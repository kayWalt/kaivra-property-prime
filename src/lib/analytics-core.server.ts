/**
 * Privileged analytics computation for the KAIVRA digital footprint system.
 *
 * The calculations were moved here verbatim from analytics.functions.ts so
 * that exactly one implementation serves both call paths:
 *   - Lovable Cloud, where SUPABASE_SERVICE_ROLE_KEY exists (direct call, and
 *     the /api/public/analytics-admin relay endpoint);
 *   - the Cloudflare-hosted frontend, which has no privileged credentials and
 *     therefore relays to Lovable Cloud instead of calling these directly.
 *
 * Authorisation is always performed by the caller before reaching this file.
 */
...
function window(input: RangeInput) {
...
export async function overviewCore(data: RangeInput) {
  ...
  const byDay = new Map<
    string,
    { day: string; sessions: number; visitors: Set<string>; events: number }
  >();
  for (const row of s) {
...
export async function activityFeedCore(
...
export async function visitorDirectoryCore(data: z.infer<typeof directorySchema>) {
  const { fromIso, toIso } = window(data);
  const supabaseAdmin = await admin();
  ...
  const visitorIds = [
    ...new Set(
      [...sessions.map((s) => s.visitor_id), ...events.map((e) => e.visitor_id)].filter(Boolean),
    ),
  ] as string[];
  ...
  for (const e of events) {
    ...
    if (
      ...
      (!person.lastSignInEvent || e.occurred_at > person.lastSignInEvent)
    ) {
  ...
  const userIds = [
    ...new Set([...people.values()].map((p) => p.userId).filter(Boolean)),
  ] as string[];
  if (userIds.length) {
...
export async function userFootprintCore(data: z.infer<typeof footprintSchema>) {
  const supabaseAdmin = await admin();
  const [{ data: profile }, { data: events }, { data: sessions }] = await Promise.all([
    ...
    supabaseAdmin
      .from("activity_events")
      .select(
        "id, occurred_at, event_type, event_category, severity, result, route, device_category, browser, os, country",
      )
    ...
    supabaseAdmin
      .from("visitor_sessions")
      .select(
        "session_id, started_at, last_seen_at, page_views, entry_page, exit_page, referrer, device_category, browser, os, country",
      )
...
export async function securitySignalsCore(data: RangeInput) {
  const { fromIso, toIso } = window(data);
  const supabaseAdmin = await admin();
  const [{ data: events }, { data: audits }] = await Promise.all([
    supabaseAdmin
      .from("activity_events")
      .select(
        "id, occurred_at, event_type, actor_label, actor_role, severity, result, route, country, visitor_id",
      )
...
export async function exportCsvCore(data: z.infer<typeof filterSchema>) {
  const { fromIso, toIso } = window(data);
  const supabaseAdmin = await admin();
  let query = supabaseAdmin
    .from("activity_events")
...
export async function retentionCore(
  data: z.infer<typeof retentionSchema>,
  userId: string,
  isSuperAdmin: boolean,
) {
  const supabaseAdmin = await admin();
  ...
  if (isSuperAdmin && (data.visitorRetentionDays || data.activityRetentionDays)) {
    await supabaseAdmin
      ...
      .update({
        ...(data.visitorRetentionDays
          ? { visitor_retention_days: data.visitorRetentionDays }
          : {}),
